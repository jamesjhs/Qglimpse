...
export const userRoles = ['root', 'institution_admin', 'institution_user', 'institution_kiosk'];
...
function assertLoginAllowed(email, ip = 'unknown') {
    const db = getDb();
    const row = db
...
export function createSessionForUser(user) {
    ...
    db.prepare('INSERT INTO auth_sessions (user_id, token_hash, expires_at, last_seen_at, created_at) VALUES (?, ?, ?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)').run(user.id, hashToken(token), expiresAt);
...
export function loginUser(input) {
    ...
    if (row.twoFaEnabled) {
        ...
        return { challengePending: true, email, expiresAt, delivery: { otpCode } };
    }
    recordLoginAttempt(email, input.ip, true);
...
export function ensureInitialAdminLogin(input) {
    ...
    const emailConflict = db
        ...
        .get(normalizedEmail, rootUserId);
    if (emailConflict) {
        throw new Error('A user with this email already exists.');
    }
    db.prepare(`UPDATE users
     SET email = ?,
         ...
         institution_id = NULL,
         deactivated_at = NULL
     WHERE id = ?`).run(normalizedEmail, rootUserId);
    ...
    const existingCredential = db
        ...
        .get(rootUserId);
    if (existingCredential) {
        db.prepare(`UPDATE user_credentials
       SET password_hash = ?, must_change_password = ?, updated_at = CURRENT_TIMESTAMP
       WHERE user_id = ?`).run(passwordHash, mustChangePassword ? 1 : 0, rootUserId);
    }
    else {
        db.prepare('INSERT INTO user_credentials (user_id, password_hash, must_change_password) VALUES (?, ?, ?)').run(rootUserId, passwordHash, mustChangePassword ? 1 : 0);
    }
