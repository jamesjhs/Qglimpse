...
export function createCustomQuestion(institutionId, input) {
    ...
    const result = db
       ...
       VALUES (?, ?, ?, ?, ?, ?, ?, ?)`)
        .run(institutionId, templateKey, input.questionType, input.prompt, JSON.stringify(input.options), input.includeInKiosk ? 1 : 0, input.isDemographic ? 1 : 0, input.displayOrder);
    const id = Number(result.lastInsertRowid);
