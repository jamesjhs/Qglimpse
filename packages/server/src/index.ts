...
function formatInstitutionValidationError(error: z.ZodError) {
...
const rateLimitJsonHandler = (_req: express.Request, res: express.Response) => {
...
const sensitiveLogKeyPattern =
  /(authorization|cookie|password|pass|secret|token|otp|session|smtp|answer|demographic|encryption|credential)/i
...
function writeStructuredLog(level: 'error' | 'warn' | 'info', message: string, details: Record<string, unknown>) {
  const payload = {
    timestamp: new Date().toISOString(),
    level,
    service: 'qglimpse-server',
...
export function createApp() {
  ...
  app.use((req, res, next) => {
    ...
    res.setHeader(
      ...
      "default-src 'self'; script-src 'self' https://challenges.cloudflare.com; style-src 'self' 'unsafe-inline'; img-src 'self' data:; connect-src 'self' https://challenges.cloudflare.com; frame-src 'self' https://challenges.cloudflare.com; child-src https://challenges.cloudflare.com; font-src 'self'; object-src 'none'; base-uri 'self'; form-action 'self'",
    )
...
function parseAdminInitArgs(argv: string[]) {
  ...
  const options: { email?: string; password?: string; mustChangePassword: boolean } = {
  ...
  if (
    process.env.npm_config_must_change_password &&
...
function printAdminInitUsage() {
  console.log(
    'Usage: npm run admin:init -- <email> <password> [must-change-password true|false]',
  )
}
