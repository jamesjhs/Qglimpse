...
Records applied schema versions. Version `1` represents the current baseline schema and compatibility migrations in `packages/server/src/db.ts`.
...
| `/guest/qr/:token` | Anonymous guest-device submission from a short-lived, single-use QR token. | Planned |
...
Kiosk session start is login-based and requires an `institution_kiosk` bearer session scoped to the requested institution. QR guest submission remains planned for `/guest/qr/:token`.
...
| `kioskRuntimeLimiter` | 1 min | 120 requests | Kiosk session start, answer, and complete endpoints |
...
Unknown `/api/...` paths return JSON `404` responses before SPA fallback routing. This keeps future QR and export routes closed until explicit handlers are implemented.
...
Qglimpse API authentication is bearer-token only. The server does not accept cookie authentication, does not set an auth cookie, and requires JavaScript to send the `Authorization: Bearer ...` header. The web client keeps the session token in React state rather than persistent browser storage, so a page refresh requires a new sign-in. Because cross-site form posts cannot attach this bearer token and cross-origin JavaScript cannot read it without an XSS foothold, CSRF protection is handled by the bearer-token design plus no-store API responses, same-origin CSP, and no CORS policy. If persistent token storage or cookie auth is added later, a synchronizer-token or double-submit CSRF control must be implemented before release.
